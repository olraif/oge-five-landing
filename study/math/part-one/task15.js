(() => {
  const prototypes = Array.isArray(window.OgeTask15DataPrototypes) ? window.OgeTask15DataPrototypes : [];
  const prototypeById = new Map(prototypes.map((item) => [item.id, item]));
  const prototypeNav = document.querySelector('[data-task15-prototypes]');
  const quiz = document.querySelector('[data-task15-quiz]');
  const submit = document.querySelector('[data-task15-submit]');
  const reset = document.querySelector('[data-task15-reset]');
  const title = document.querySelector('[data-task15-title]');
  const score = document.querySelector('[data-task15-score]');
  const currentTotal = document.querySelector('[data-task15-current-total]');
  const total = document.querySelector('[data-task15-total]');
  const percent = document.querySelector('[data-task15-percent]');
  const note = document.querySelector('[data-task15-note]');
  const result = document.querySelector('[data-task15-result]');
  if (!prototypeNav || !quiz || !submit || !reset || !prototypes.length) return;

  const storagePrefix = 'ogeTrainer:v3:math:task15:';
  const resetStoragePrefix = 'ogeTrainer:v3:math:task15Reset:';
  const accountStorage = window.OgeProgressModel?.createAccountProgressStorage(localStorage);
  const cloudPath = ['trainer_progress', 'math', 'task15'];
  const progressPath = ['math', 'task15'];
  ...
  const getSaved = (id) => {
    const cloudAttempt = cloudUser?.user_metadata?.trainer_progress?.math?.task15?.[id];
  ...
  const renderNav = () => {
    prototypeNav.innerHTML = prototypes.map((proto) => {
      const saved = validSaved(getSaved(proto.id), proto);
      const correct = saved?.correctIds?.length || 0;
      return `<button type="button" class="prototype-progress ${prototypeState(proto)} ${proto.id === activeId ? 'is-current' : ''}" data-task15-prototype="${proto.id}"><b>${proto.id}</b><span>${correct}/${proto.items.length}</span></button>`;
    }).join('');
    prototypeNav.querySelectorAll('[data-task15-prototype]').forEach((button) => button.addEventListener('click', () => {
      activeId = button.dataset.task15Prototype;
      render();
  ...
  const saveCloud = async (payload) => {
    ...
    try {
      ...
      const next = {
        ...
        math: {
          ...(current.math || {}),
          task15: { ...(current.math?.task15 || {}), [ownedPayload.prototype]: ownedPayload },
        },
      };
      ...
      console.warn('Не удалось сохранить прогресс №15', error);
    }
  };
  ...
  const checkPrototype = async () => {
    ...
    renderOverall();
    saveCloud(payload);
    if (typeof window.ym === 'function' && window.METRIKA_COUNTER_ID) window.ym(window.METRIKA_COUNTER_ID, 'reachGoal', 'MATH_TASK15_TEST');
  };
  const resetPrototype = async () => {
    ...
    if (cloudUser && resetKey && window.ogeSupabase) {
      ...
      } catch (error) { console.warn('Не удалось синхронизировать сброс прогресса №15', error); }
    }
