...
PAGE = BASE + "/sources/FIPI_OGE_MATH/progressions"
API = BASE + "/api/analog/FIPI_OGE_MATH/progressions"
ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "study" / "math" / "part-one"
DRAWINGS_OUT = OUT / "task14-drawings"
EXPECTED_TOTALS = [10, 10, 10, 10, 10, 10, 20, 7, 10, 10, 10]
...
def localize_drawings(task_html):
    ...
    def replace_source(match):
        ...
        relative = source.split("/progressions/", 1)[-1]
        safe_name = relative.replace("/", "-")
        target = DRAWINGS_OUT / safe_name
        if not target.exists():
            target.write_bytes(fetch_bytes(BASE + source))
        return f'src="task14-drawings/{safe_name}"'
    return re.sub(r'src="(/drawings/FIPI_OGE_MATH/progressions/[^"]+)"', replace_source, task_html)
...
def collect():
    ...
    for index, article in enumerate(bases, 1):
        ...
        result.append({"id": pid, "title": repair_text(title), "source": "MathStart / FIPI_OGE_MATH / progressions", "items": items})
        print(f"{pid}: {len(items)}")
    total = sum(len(row["items"]) for row in result)
    if total != 117: raise RuntimeError(f"Expected 117 tasks, collected {total}")
    return result

def write_data(rows):
    for path in OUT.glob("task14-data-*.js"): path.unlink()
    for index, row in enumerate(rows, 1):
        payload = json.dumps(row, ensure_ascii=False, separators=(",", ":"))
        text = "window.OgeTask14DataPrototypes = window.OgeTask14DataPrototypes || [];\n" + f"window.OgeTask14DataPrototypes.push({payload});\n"
        (OUT / f"task14-data-{index:02d}.js").write_text(text, encoding="utf-8")
