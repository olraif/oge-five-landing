(() => {
  const prototypes = Array.isArray(window.OgeTask17DataPrototypes) ? window.OgeTask17DataPrototypes : [];
  const prototypeById = new Map(prototypes.map((item) => [item.id, item]));
  const prototypeNav = document.querySelector('[data-task17-prototypes]');
  const quiz = document.querySelector('[data-task17-quiz]');
  const submit = document.querySelector('[data-task17-submit]');
  const title = document.querySelector('[data-task17-title]');
  const score = document.querySelector('[data-task17-score]');
  const currentTotal = document.querySelector('[data-task17-current-total]');
  const total = document.querySelector('[data-task17-total]');
  const percent = document.querySelector('[data-task17-percent]');
  const note = document.querySelector('[data-task17-note]');
  const result = document.querySelector('[data-task17-result]');
  if (!prototypeNav || !quiz || !prototypes.length) return;

  const storagePrefix = 'ogeTrainer:v3:math:task17:';
  const accountStorage = window.OgeProgressModel?.createAccountProgressStorage(localStorage);
  const cloudPath = ['trainer_progress', 'math', 'task17'];
  ...
  const getSaved = (id) => {
    const cloudAttempt = cloudUser?.user_metadata?.trainer_progress?.math?.task17?.[id];
  ...
  const renderNav = () => {
    prototypeNav.innerHTML = prototypes.map((proto) => {
      const saved = validSaved(getSaved(proto.id), proto);
      const correct = saved?.correctIds?.length || 0;
      return `<button type="button" class="prototype-progress ${prototypeState(proto)} ${proto.id === activeId ? 'is-current' : ''}" data-task17-prototype="${proto.id}"><b>${proto.id}</b><span>${correct}/${proto.items.length}</span></button>`;
    }).join('');
    prototypeNav.querySelectorAll('[data-task17-prototype]').forEach((button) => button.addEventListener('click', () => {
      activeId = button.dataset.task17Prototype;
      render();
  ...
  const saveCloud = async (payload) => {
    ...
    try {
      // Р‘РµСЂС‘Рј СЃРІРµР¶РёРµ РјРµС‚Р°РґР°РЅРЅС‹Рµ РїРµСЂРµРґ РєР°Р¶РґС‹Рј СЃРѕС…СЂР°РЅРµРЅРёРµРј, С‡С‚РѕР±С‹ РѕС‚РІРµС‚С‹
      // РЅРµСЃРєРѕР»СЊРєРёС… РїСЂРѕС‚РѕС‚РёРїРѕРІ РЅРµ РїРµСЂРµР·Р°РїРёСЃС‹РІР°Р»Рё РґСЂСѓРі РґСЂСѓРіР°.
      ...
      const next = {
        ...
        math: {
          ...(current.math || {}),
          task17: { ...(current.math?.task17 || {}), [ownedPayload.prototype]: ownedPayload },
        },
      };
      ...
      console.warn('РќРµ СѓРґР°Р»РѕСЃСЊ СЃРѕС…СЂР°РЅРёС‚СЊ РїСЂРѕРіСЂРµСЃСЃ в„–17', error);
    }
  };
  const loadCloud = async () => {
    ...
    try {
      ...
      const cloudValue = cloudUser?.user_metadata?.trainer_progress?.math?.task17?.[activeId];
      const proto = prototypeById.get(activeId);
  ...
  const checkPrototype = async () => {
    ...
    renderOverall();
    saveCloud(payload);
    if (typeof window.ym === 'function' && window.METRIKA_COUNTER_ID) window.ym(window.METRIKA_COUNTER_ID, 'reachGoal', 'MATH_TASK17_TEST');
  };
