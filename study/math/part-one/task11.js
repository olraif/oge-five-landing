(() => {
  const prototypes = Array.isArray(window.OgeTask11DataPrototypes) ? window.OgeTask11DataPrototypes : [];
  const prototypeById = new Map(prototypes.map((item) => [item.id, item]));
  const prototypeNav = document.querySelector('[data-task11-prototypes]');
  const quiz = document.querySelector('[data-task11-quiz]');
  const submit = document.querySelector('[data-task11-submit]');
  const reset = document.querySelector('[data-task11-reset]');
  const title = document.querySelector('[data-task11-title]');
  const score = document.querySelector('[data-task11-score]');
  const currentTotal = document.querySelector('[data-task11-current-total]');
  const total = document.querySelector('[data-task11-total]');
  const percent = document.querySelector('[data-task11-percent]');
  const note = document.querySelector('[data-task11-note]');
  const result = document.querySelector('[data-task11-result]');
  if (!prototypeNav || !quiz || !submit || !reset || !prototypes.length) return;

  const storagePrefix = 'ogeTrainer:v3:math:task11:';
  const resetStoragePrefix = 'ogeTrainer:v3:math:task11Reset:';
  ...
  const progressPath = ['math', 'task11'];
  ...
  const renderNav = () => {
    prototypeNav.innerHTML = prototypes.map((proto) => {
      const saved = validSaved(getSaved(proto.id), proto);
      const correct = saved?.correctIds?.length || 0;
      return `<button type="button" class="prototype-progress ${prototypeState(proto)} ${proto.id === activeId ? 'is-current' : ''}" data-task11-prototype="${proto.id}"><b>${proto.id}</b><span>${correct}/${proto.items.length}</span></button>`;
    }).join('');
    prototypeNav.querySelectorAll('[data-task11-prototype]').forEach((button) => button.addEventListener('click', () => {
      activeId = button.dataset.task11Prototype;
      render();
  ...
  const saveCloud = async (payload) => {
    ...
    try {
      ...
      console.warn('Не удалось сохранить прогресс №11', error);
    }
  };
  ...
  const checkPrototype = async () => {
    ...
    renderOverall();
    saveCloud(payload);
    if (typeof window.ym === 'function' && window.METRIKA_COUNTER_ID) window.ym(window.METRIKA_COUNTER_ID, 'reachGoal', 'MATH_TASK11_TEST');
  };
  const resetPrototype = async () => {
    ...
    if (cloudUser && resetKey && window.ogeSupabase) {
      ...
      } catch (error) { console.warn('Не удалось синхронизировать сброс прогресса №11', error); }
    }
