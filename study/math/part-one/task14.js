(() => {
  const prototypes = Array.isArray(window.OgeTask14DataPrototypes) ? window.OgeTask14DataPrototypes : [];
  const prototypeById = new Map(prototypes.map((item) => [item.id, item]));
  const prototypeNav = document.querySelector('[data-task14-prototypes]');
  const quiz = document.querySelector('[data-task14-quiz]');
  const submit = document.querySelector('[data-task14-submit]');
  const reset = document.querySelector('[data-task14-reset]');
  const title = document.querySelector('[data-task14-title]');
  const score = document.querySelector('[data-task14-score]');
  const currentTotal = document.querySelector('[data-task14-current-total]');
  const total = document.querySelector('[data-task14-total]');
  const percent = document.querySelector('[data-task14-percent]');
  const note = document.querySelector('[data-task14-note]');
  const result = document.querySelector('[data-task14-result]');
  if (!prototypeNav || !quiz || !submit || !reset || !prototypes.length) return;

  const storagePrefix = 'ogeTrainer:v3:math:task14:';
  const resetStoragePrefix = 'ogeTrainer:v3:math:task14Reset:';
  const accountStorage = window.OgeProgressModel?.createAccountProgressStorage(localStorage);
  const cloudPath = ['trainer_progress', 'math', 'task14'];
  const progressPath = ['math', 'task14'];
  ...
  const getSaved = (id) => {
    const cloudAttempt = cloudUser?.user_metadata?.trainer_progress?.math?.task14?.[id];
  ...
  const renderNav = () => {
    prototypeNav.innerHTML = prototypes.map((proto) => {
      const saved = validSaved(getSaved(proto.id), proto);
      const correct = saved?.correctIds?.length || 0;
      return `<button type="button" class="prototype-progress ${prototypeState(proto)} ${proto.id === activeId ? 'is-current' : ''}" data-task14-prototype="${proto.id}"><b>${proto.id}</b><span>${correct}/${proto.items.length}</span></button>`;
    }).join('');
    prototypeNav.querySelectorAll('[data-task14-prototype]').forEach((button) => button.addEventListener('click', () => {
      activeId = button.dataset.task14Prototype;
      render();
  ...
  const saveCloud = async (payload) => {
    ...
    try {
      ...
      const next = {
        ...
        math: {
          ...(current.math || {}),
          task14: { ...(current.math?.task14 || {}), [ownedPayload.prototype]: ownedPayload },
        },
      };
      ...
      console.warn('Не удалось сохранить прогресс №14', error);
    }
  };
  ...
  const checkPrototype = async () => {
    ...
    renderOverall();
    saveCloud(payload);
    if (typeof window.ym === 'function' && window.METRIKA_COUNTER_ID) window.ym(window.METRIKA_COUNTER_ID, 'reachGoal', 'MATH_TASK14_TEST');
  };
  const resetPrototype = async () => {
    ...
    if (cloudUser && resetKey && window.ogeSupabase) {
      ...
      } catch (error) { console.warn('Не удалось синхронизировать сброс прогресса №14', error); }
    }
