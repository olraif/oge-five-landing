(() => {
  const prototypes = Array.isArray(window.OgeTask18DataPrototypes) ? window.OgeTask18DataPrototypes : [];
  const prototypeById = new Map(prototypes.map((item) => [item.id, item]));
  const prototypeNav = document.querySelector('[data-task18-prototypes]');
  const quiz = document.querySelector('[data-task18-quiz]');
  const submit = document.querySelector('[data-task18-submit]');
  const title = document.querySelector('[data-task18-title]');
  const score = document.querySelector('[data-task18-score]');
  const currentTotal = document.querySelector('[data-task18-current-total]');
  const total = document.querySelector('[data-task18-total]');
  const percent = document.querySelector('[data-task18-percent]');
  const note = document.querySelector('[data-task18-note]');
  const result = document.querySelector('[data-task18-result]');
  if (!prototypeNav || !quiz || !prototypes.length) return;

  const storagePrefix = 'ogeTrainer:v3:math:task18:';
  const accountStorage = window.OgeProgressModel?.createAccountProgressStorage(localStorage);
  const cloudPath = ['trainer_progress', 'math', 'task18'];
  ...
  const getSaved = (id) => {
    const cloudAttempt = cloudUser?.user_metadata?.trainer_progress?.math?.task18?.[id];
  ...
  const renderNav = () => {
    prototypeNav.innerHTML = prototypes.map((proto) => {
      const saved = validSaved(getSaved(proto.id), proto);
      const correct = saved?.correctIds?.length || 0;
      return `<button type="button" class="prototype-progress ${prototypeState(proto)} ${proto.id === activeId ? 'is-current' : ''}" data-task18-prototype="${proto.id}"><b>${proto.id}</b><span>${correct}/${proto.items.length}</span></button>`;
    }).join('');
    prototypeNav.querySelectorAll('[data-task18-prototype]').forEach((button) => button.addEventListener('click', () => {
      activeId = button.dataset.task18Prototype;
      render();
  ...
  const saveCloud = async (payload) => {
    ...
    try {
      ...
      const next = {
        ...
        math: {
          ...(current.math || {}),
          task18: { ...(current.math?.task18 || {}), [ownedPayload.prototype]: ownedPayload },
        },
      };
  ...
  const loadCloud = async () => {
    ...
    try {
      ...
      const cloudValue = cloudUser?.user_metadata?.trainer_progress?.math?.task18?.[activeId];
      const proto = prototypeById.get(activeId);
  ...
  const checkPrototype = async () => {
    ...
    renderOverall();
    saveCloud(payload);
    if (typeof window.ym === 'function' && window.METRIKA_COUNTER_ID) window.ym(window.METRIKA_COUNTER_ID, 'reachGoal', 'MATH_TASK18_TEST');
  };
