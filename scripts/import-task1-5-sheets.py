...
OUTPUT = PART_ONE / "task1-5-sheets-data.js"
...
def localize_drawings(markup):
    ...
    for source in sorted(set(re.findall(
        r'/drawings/FIPI_OGE_MATH/real_math/(papers-\d+\.svg)', markup,
    ))):
...
def collect():
    ...
    heading = soup.find(id="subtopic-1ndash5_listy")
    ...
    if len(analogs) != 4:
        raise RuntimeError(f"Expected 4 sheet analogs, collected {len(analogs)}")
    return [{
        "id": "sheets-4.1",
        "number": "4.1",
        "title": "Листы",
...
if __name__ == "__main__":
    rows = collect()
    write_data(rows)
    print(f"Collected {len(rows[0]['analogs'])} sheet analogs.")
