...
PAGE = BASE + "/sources/FIPI_OGE_MATH/poligons"
API = BASE + "/api/analog/FIPI_OGE_MATH/poligons"
ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "study" / "math" / "part-one"
DRAWINGS_OUT = OUT / "task17-drawings"
...
def localize_drawings(task_html):
    ...
    def replace_source(match):
        ...
        relative = source.split("/poligons/", 1)[-1]
        safe_name = relative.replace("/", "-")
        target = DRAWINGS_OUT / safe_name
        if not target.exists():
            target.write_bytes(fetch_bytes(BASE + source))
        return f'src="task17-drawings/{safe_name}"'

    return re.sub(r'src="(/drawings/FIPI_OGE_MATH/poligons/[^"]+)"', replace_source, task_html)
...
def collect():
    ...
    if len(bases) != 40:
        raise RuntimeError(f"Expected 40 prototypes, found {len(bases)}")

    result = []
    global_ids = set()
    for index, article in enumerate(bases, 1):
        prototype_id = f"17.{index}"
        ...
        result.append({
            ...
            "source": "MathStart / FIPI_OGE_MATH / poligons",
            "items": items,
        })
    ...
    if total != 320:
        raise RuntimeError(f"Expected 320 tasks, collected {total}")
    return result


def write_data(rows):
    for path in OUT.glob("task17-data-*.js"):
        path.unlink()
    for index, row in enumerate(rows, 1):
        payload = json.dumps(row, ensure_ascii=False, separators=(",", ":"))
        text = "window.OgeTask17DataPrototypes = window.OgeTask17DataPrototypes || [];\n"
        text += f"window.OgeTask17DataPrototypes.push({payload});\n"
        (OUT / f"task17-data-{index:02d}.js").write_text(text, encoding="utf-8")
