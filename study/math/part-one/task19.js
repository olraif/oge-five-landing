(() => {
  const prototypes = Array.isArray(window.OgeTask19DataPrototypes) ? window.OgeTask19DataPrototypes : [];
  const prototypeById = new Map(prototypes.map((item) => [item.id, item]));
  const prototypeNav = document.querySelector('[data-task19-prototypes]');
  const quiz = document.querySelector('[data-task19-quiz]');
  const submit = document.querySelector('[data-task19-submit]');
  const title = document.querySelector('[data-task19-title]');
  const score = document.querySelector('[data-task19-score]');
  const currentTotal = document.querySelector('[data-task19-current-total]');
  const total = document.querySelector('[data-task19-total]');
  const percent = document.querySelector('[data-task19-percent]');
  const note = document.querySelector('[data-task19-note]');
  const result = document.querySelector('[data-task19-result]');
  if (!prototypeNav || !quiz || !prototypes.length) return;

  const storagePrefix = 'ogeTrainer:v3:math:task19:';
  const accountStorage = window.OgeProgressModel?.createAccountProgressStorage(localStorage);
  const cloudPath = ['trainer_progress', 'math', 'task19'];
  ...
  const getSaved = (id) => {
    const cloudAttempt = cloudUser?.user_metadata?.trainer_progress?.math?.task19?.[id];
  ...
  const renderNav = () => {
    prototypeNav.innerHTML = prototypes.map((proto) => {
      const saved = validSaved(getSaved(proto.id), proto);
      const correct = saved?.correctIds?.length || 0;
      return `<button type="button" class="prototype-progress ${prototypeState(proto)} ${proto.id === activeId ? 'is-current' : ''}" data-task19-prototype="${proto.id}"><b>${proto.id}</b><span>${correct}/${proto.items.length}</span></button>`;
    }).join('');
    prototypeNav.querySelectorAll('[data-task19-prototype]').forEach((button) => button.addEventListener('click', () => {
      activeId = button.dataset.task19Prototype;
      render();
  ...
  const saveCloud = async (payload) => {
    ...
    try {
      // Read fresh metadata before saving so prototype answers do not overwrite each other.
      ...
      const next = {
        ...
        math: {
          ...(current.math || {}),
          task19: { ...(current.math?.task19 || {}), [ownedPayload.prototype]: ownedPayload },
        },
      };
      ...
      console.warn('Не удалось сохранить прогресс задания №19', error);
    }
  };
  const loadCloud = async () => {
    ...
    try {
      ...
      const cloudValue = cloudUser?.user_metadata?.trainer_progress?.math?.task19?.[activeId];
      const proto = prototypeById.get(activeId);
  ...
  const checkPrototype = async () => {
    ...
    renderOverall();
    saveCloud(payload);
    if (typeof window.ym === 'function' && window.METRIKA_COUNTER_ID) window.ym(window.METRIKA_COUNTER_ID, 'reachGoal', 'MATH_TASK19_TEST');
  };
