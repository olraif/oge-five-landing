(() => {
  const prototypes = Array.isArray(window.OgeTask16DataPrototypes) ? window.OgeTask16DataPrototypes : [];
  const prototypeById = new Map(prototypes.map((item) => [item.id, item]));
  const prototypeNav = document.querySelector('[data-task16-prototypes]');
  const quiz = document.querySelector('[data-task16-quiz]');
  const submit = document.querySelector('[data-task16-submit]');
  const title = document.querySelector('[data-task16-title]');
  const score = document.querySelector('[data-task16-score]');
  const currentTotal = document.querySelector('[data-task16-current-total]');
  const total = document.querySelector('[data-task16-total]');
  const percent = document.querySelector('[data-task16-percent]');
  const note = document.querySelector('[data-task16-note]');
  const result = document.querySelector('[data-task16-result]');
  if (!prototypeNav || !quiz || !prototypes.length) return;

  const storagePrefix = 'ogeTrainer:v3:math:task16:';
  const accountStorage = window.OgeProgressModel?.createAccountProgressStorage(localStorage);
  const cloudPath = ['trainer_progress', 'math', 'task16'];
  ...
  const getSaved = (id) => {
    const cloudAttempt = cloudUser?.user_metadata?.trainer_progress?.math?.task16?.[id];
  ...
  const renderNav = () => {
    prototypeNav.innerHTML = prototypes.map((proto) => {
      const saved = validSaved(getSaved(proto.id), proto);
      const correct = saved?.correctIds?.length || 0;
      return `<button type="button" class="prototype-progress ${prototypeState(proto)} ${proto.id === activeId ? 'is-current' : ''}" data-task16-prototype="${proto.id}"><b>${proto.id}</b><span>${correct}/${proto.items.length}</span></button>`;
    }).join('');
    prototypeNav.querySelectorAll('[data-task16-prototype]').forEach((button) => button.addEventListener('click', () => {
      activeId = button.dataset.task16Prototype;
      render();
  ...
  const saveCloud = async (payload) => {
    ...
    try {
      ...
      const next = {
        ...
        math: {
          ...(current.math || {}),
          task16: { ...(current.math?.task16 || {}), [ownedPayload.prototype]: ownedPayload },
        },
      };
      ...
      console.warn('Не удалось сохранить прогресс №16', error);
    }
  };
  const loadCloud = async () => {
    ...
    try {
      ...
      const cloudValue = cloudUser?.user_metadata?.trainer_progress?.math?.task16?.[activeId];
      const proto = prototypeById.get(activeId);
  ...
  const checkPrototype = async () => {
    ...
    renderOverall();
    saveCloud(payload);
    if (typeof window.ym === 'function' && window.METRIKA_COUNTER_ID) window.ym(window.METRIKA_COUNTER_ID, 'reachGoal', 'MATH_TASK16_TEST');
  };
