...
PAGE = BASE + "/sources/FIPI_OGE_MATH/equations"
API = BASE + "/api/analog/FIPI_OGE_MATH/equations"
...
def collect():
    ...
    if len(bases) != 14:
        raise RuntimeError(f"Expected 14 prototypes, found {len(bases)}")
    result, global_ids = [], set()
    quotas = [10] * 10 + [7, 5, 6, 6]
    if sum(quotas) != 124:
        raise RuntimeError("Task 9 quota must contain exactly 124 tasks")
    for index, base in enumerate(bases, 1):
        pid, base_id = f"9.{index}", base["internal_id"]
        ...
        result.append({"id": pid, "title": base["title"],
                       "source": "MathStart / FIPI_OGE_MATH / equations", "items": items})
        print(f"{pid}: {len(items)}")
    total = sum(len(row["items"]) for row in result)
    if total != 124:
        raise RuntimeError(f"Expected 124 tasks, collected {total}")
    return result

def write_data(rows):
    for path in OUT.glob("task9-data-*.js"):
        path.unlink()
    for index, row in enumerate(rows, 1):
        payload = json.dumps(row, ensure_ascii=False, separators=(",", ":"))
        text = ("window.OgeTask9DataPrototypes = window.OgeTask9DataPrototypes || [];\n"
                f"window.OgeTask9DataPrototypes.push({payload});\n")
        (OUT / f"task9-data-{index:02d}.js").write_text(text, encoding="utf-8")
