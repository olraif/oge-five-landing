(() => {
  const prototypes = Array.isArray(window.OgeTask13DataPrototypes) ? window.OgeTask13DataPrototypes : [];
  const prototypeById = new Map(prototypes.map((item) => [item.id, item]));
  const prototypeNav = document.querySelector('[data-task13-prototypes]');
  const quiz = document.querySelector('[data-task13-quiz]');
  const submit = document.querySelector('[data-task13-submit]');
  const reset = document.querySelector('[data-task13-reset]');
  const title = document.querySelector('[data-task13-title]');
  const score = document.querySelector('[data-task13-score]');
  const currentTotal = document.querySelector('[data-task13-current-total]');
  const total = document.querySelector('[data-task13-total]');
  const percent = document.querySelector('[data-task13-percent]');
  const note = document.querySelector('[data-task13-note]');
  const result = document.querySelector('[data-task13-result]');
  if (!prototypeNav || !quiz || !submit || !reset || !prototypes.length) return;

  const storagePrefix = 'ogeTrainer:v3:math:task13:';
  const resetStoragePrefix = 'ogeTrainer:v3:math:task13Reset:';
  const accountStorage = window.OgeProgressModel?.createAccountProgressStorage(localStorage);
  const cloudPath = ['trainer_progress', 'math', 'task13'];
  const progressPath = ['math', 'task13'];
  ...
  const getSaved = (id) => {
    const cloudAttempt = cloudUser?.user_metadata?.trainer_progress?.math?.task13?.[id];
  ...
  const renderNav = () => {
    prototypeNav.innerHTML = prototypes.map((proto) => {
      const saved = validSaved(getSaved(proto.id), proto);
      const correct = saved?.correctIds?.length || 0;
      return `<button type="button" class="prototype-progress ${prototypeState(proto)} ${proto.id === activeId ? 'is-current' : ''}" data-task13-prototype="${proto.id}"><b>${proto.id}</b><span>${correct}/${proto.items.length}</span></button>`;
    }).join('');
    prototypeNav.querySelectorAll('[data-task13-prototype]').forEach((button) => button.addEventListener('click', () => {
      activeId = button.dataset.task13Prototype;
      render();
  ...
  const saveCloud = async (payload) => {
    ...
    try {
      ...
      const next = {
        ...
        math: {
          ...(current.math || {}),
          task13: { ...(current.math?.task13 || {}), [ownedPayload.prototype]: ownedPayload },
        },
      };
      ...
      console.warn('Не удалось сохранить прогресс №13', error);
    }
  };
  ...
  const checkPrototype = async () => {
    ...
    renderOverall();
    saveCloud(payload);
    if (typeof window.ym === 'function' && window.METRIKA_COUNTER_ID) window.ym(window.METRIKA_COUNTER_ID, 'reachGoal', 'MATH_TASK13_TEST');
  };
  const resetPrototype = async () => {
    ...
    if (cloudUser && resetKey && window.ogeSupabase) {
      ...
      } catch (error) { console.warn('Не удалось синхронизировать сброс прогресса №13', error); }
    }
