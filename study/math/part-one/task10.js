(() => {
  const prototypes = Array.isArray(window.OgeTask10DataPrototypes) ? window.OgeTask10DataPrototypes : [];
  ...
  const total = document.querySelector('[data-task10-total]');
  ...
  const renderNav = () => {
    prototypeNav.innerHTML = prototypes.map((proto) => {
      const saved = validSaved(getSaved(proto.id), proto);
      const correct = saved?.correctIds?.length || 0;
      return `<button type="button" class="prototype-progress ${prototypeState(proto)} ${proto.id === activeId ? 'is-current' : ''}" data-task10-prototype="${proto.id}"><b>${proto.id}</b><span>${correct}/${proto.items.length}</span></button>`;
    }).join('');
    prototypeNav.querySelectorAll('[data-task10-prototype]').forEach((button) => button.addEventListener('click', () => {
      activeId = button.dataset.task10Prototype;
      render();
  ...
  const saveCloud = async (payload) => {
    ...
    try {
      ...
      const next = {
        ...
        math: {
          ...(current.math || {}),
          task10: { ...(current.math?.task10 || {}), [ownedPayload.prototype]: ownedPayload },
        },
      };
