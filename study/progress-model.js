(function initProgressModel(root, factory) {
  ...
  const TASK6_ANSWER_KEYS = Object.freeze({
    '6.1': ['14,2','9,1','13,3','12','13','14,6','16,6','17,9','12,4'],
    '6.2': ['0,8','-3,7','2,2','-3,8','-2,9','3,2','1,3','4','-0,2'],
    '6.3': ['15,75','14,26','9,92','28,6','35,51','55,44','34,83','63,19','95,04'],
    '6.4': ['4','21','17','4','3','24','33','6,75','13'],
    '6.5': ['2,35','2,2','1,2','1,75','0,86','2,05','1,8','1,05','1,1'],
    '6.6': ['-2,05','-4,2','-0,57','-0,58','-0,95','0,06','-2,35','-0,16','-2,51'],
    '6.7': ['2','0,84375','4,21875','0,48','0,4','0,6','11,25','2,8','0,4'],
    '6.8': ['28,125','1,4','2,1','2,2','1,4','5,6','9,8','1,2','3,0625'],
    '6.9': ['20','3','21','55','29','5'],
    '6.10': ['14','19,8','15,75','28','180','157,5','77'],
  });
...
const TASK14_TOTALS = Object.freeze({
  ...
  const TASK19_TOTALS = Object.freeze({ '19.1': 88, '19.2': 63 });
...
const summarizeTask15Prototype = (key, attempt = {}) => {
  ...
  const summarizeTask19Prototype = (key, attempt = {}) => {
    const total = TASK19_TOTALS[key] || Number(attempt.total) || 0;
    const correctIds = Array.isArray(attempt.correctIds) ? attempt.correctIds : [];
    const answeredIds = Array.isArray(attempt.answeredIds) ? attempt.answeredIds : Object.keys(attempt.answers || {}).filter((id) => isAnswered(attempt.answers[id]));
    const correct = Math.min(total, correctIds.length);
    const answered = Math.min(total, Math.max(correct, answeredIds.length));
  ...
  const buildTask19Summary = (task19Progress = {}) => {
    const prototypes = Object.keys(TASK19_TOTALS).map((key) => summarizeTask19Prototype(key, task19Progress?.[key]));
    const totals = prototypes.reduce((result, prototype) => ({
      total: result.total + prototype.total,
      correct: result.correct + prototype.correct,
      wrong: result.wrong + prototype.wrong,
