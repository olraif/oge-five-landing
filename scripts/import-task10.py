...
PAGE = BASE + "/sources/FIPI_OGE_MATH/probabilities"
...
def collect():
    ...
    if len(bases) != 18:
        raise RuntimeError(f"Expected 18 prototypes, found {len(bases)}")
    result, global_ids = [], set()
    for index, base in enumerate(bases, 1):
        pid, base_id = f"10.{index}", base["internal_id"]
    ...
    if total != 217:
        raise RuntimeError(f"Expected 217 tasks, collected {total}")
    return result

def write_data(rows):
    for path in OUT.glob("task10-data-*.js"):
        path.unlink()
    for index, row in enumerate(rows, 1):
        payload = json.dumps(row, ensure_ascii=False, separators=(",", ":"))
        text = ("window.OgeTask10DataPrototypes = window.OgeTask10DataPrototypes || [];\n"
                f"window.OgeTask10DataPrototypes.push({payload});\n")
        (OUT / f"task10-data-{index:02d}.js").write_text(text, encoding="utf-8")
