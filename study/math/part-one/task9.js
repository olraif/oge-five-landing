(() => {
  const prototypes = Array.isArray(window.OgeTask9DataPrototypes) ? window.OgeTask9DataPrototypes : [];
  const prototypeById = new Map(prototypes.map((item) => [item.id, item]));
  const prototypeNav = document.querySelector('[data-task9-prototypes]');
  const quiz = document.querySelector('[data-task9-quiz]');
  const submit = document.querySelector('[data-task9-submit]');
  const reset = document.querySelector('[data-task9-reset]');
  const title = document.querySelector('[data-task9-title]');
  const score = document.querySelector('[data-task9-score]');
  const currentTotal = document.querySelector('[data-task9-current-total]');
  const total = document.querySelector('[data-task9-total]');
  const percent = document.querySelector('[data-task9-percent]');
  const note = document.querySelector('[data-task9-note]');
  const result = document.querySelector('[data-task9-result]');
  if (!prototypeNav || !quiz || !submit || !reset || !prototypes.length) return;

  const storagePrefix = 'ogeTrainer:v3:math:task9:';
  const resetStoragePrefix = 'ogeTrainer:v3:math:task9Reset:';
  const accountStorage = window.OgeProgressModel?.createAccountProgressStorage(localStorage);
  const cloudPath = ['trainer_progress', 'math', 'task9'];
  const progressPath = ['math', 'task9'];
  ...
  const getSaved = (id) => {
    const cloudAttempt = cloudUser?.user_metadata?.trainer_progress?.math?.task9?.[id];
  ...
  const renderNav = () => {
    prototypeNav.innerHTML = prototypes.map((proto) => {
      const saved = validSaved(getSaved(proto.id), proto);
      const correct = saved?.correctIds?.length || 0;
      return `<button type="button" class="prototype-progress ${prototypeState(proto)} ${proto.id === activeId ? 'is-current' : ''}" data-task9-prototype="${proto.id}"><b>${proto.id}</b><span>${correct}/${proto.items.length}</span></button>`;
    }).join('');
    prototypeNav.querySelectorAll('[data-task9-prototype]').forEach((button) => button.addEventListener('click', () => {
      activeId = button.dataset.task9Prototype;
      render();
  ...
  const saveCloud = async (payload) => {
    ...
    try {
      ...
      const next = {
        ...
        math: {
          ...(current.math || {}),
          task9: { ...(current.math?.task9 || {}), [ownedPayload.prototype]: ownedPayload },
        },
      };
  ...
  const checkPrototype = async () => {
    ...
    renderOverall();
    saveCloud(payload);
    if (typeof window.ym === 'function' && window.METRIKA_COUNTER_ID) window.ym(window.METRIKA_COUNTER_ID, 'reachGoal', 'MATH_TASK9_TEST');
  };
  const resetPrototype = async () => {
    ...
    if (cloudUser && resetKey && window.ogeSupabase) {
      ...
      } catch (error) { console.warn('Не удалось синхронизировать сброс прогресса №9', error); }
    }
