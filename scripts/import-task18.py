...
PAGE = BASE + "/sources/FIPI_OGE_MATH/checkered_paper"
API = BASE + "/api/analog/FIPI_OGE_MATH/checkered_paper"
ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "study" / "math" / "part-one"
DRAWINGS_OUT = OUT / "task18-drawings"
...
def localize_drawings(task_html):
    ...
    def replace_source(match):
        ...
        relative = source.split("/checkered_paper/", 1)[-1]
        safe_name = relative.replace("/", "-")
        target = DRAWINGS_OUT / safe_name
        if not target.exists():
            target.write_bytes(fetch_bytes(BASE + source))
        return f'src="task18-drawings/{safe_name}"'

    return re.sub(r'src="(/drawings/FIPI_OGE_MATH/checkered_paper/[^"]+)"', replace_source, task_html)
...
def collect():
    ...
    if len(bases) != 13:
        raise RuntimeError(f"Expected 13 prototypes, found {len(bases)}")

    result = []
    global_ids = set()
    for index, article in enumerate(bases, 1):
        prototype_id = f"18.{index}"
        ...
        result.append({
            ...
            "source": "MathStart / FIPI_OGE_MATH / checkered_paper",
            "items": items,
        })
    ...
    if total != 158:
        raise RuntimeError(f"Expected 158 tasks, collected {total}")
    return result


def write_data(rows):
    for path in OUT.glob("task18-data-*.js"):
        path.unlink()
    for index, row in enumerate(rows, 1):
        payload = json.dumps(row, ensure_ascii=False, separators=(",", ":"))
        text = "window.OgeTask18DataPrototypes = window.OgeTask18DataPrototypes || [];\n"
        text += f"window.OgeTask18DataPrototypes.push({payload});\n"
        (OUT / f"task18-data-{index:02d}.js").write_text(text, encoding="utf-8")
