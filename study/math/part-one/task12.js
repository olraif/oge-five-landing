(() => {
  const prototypes = Array.isArray(window.OgeTask12DataPrototypes) ? window.OgeTask12DataPrototypes : [];
  const prototypeById = new Map(prototypes.map((item) => [item.id, item]));
  const prototypeNav = document.querySelector('[data-task12-prototypes]');
  const quiz = document.querySelector('[data-task12-quiz]');
  const submit = document.querySelector('[data-task12-submit]');
  const reset = document.querySelector('[data-task12-reset]');
  const title = document.querySelector('[data-task12-title]');
  const score = document.querySelector('[data-task12-score]');
  const currentTotal = document.querySelector('[data-task12-current-total]');
  const total = document.querySelector('[data-task12-total]');
  const percent = document.querySelector('[data-task12-percent]');
  const note = document.querySelector('[data-task12-note]');
  const result = document.querySelector('[data-task12-result]');
  if (!prototypeNav || !quiz || !submit || !reset || !prototypes.length) return;

  const storagePrefix = 'ogeTrainer:v3:math:task12:';
  const resetStoragePrefix = 'ogeTrainer:v3:math:task12Reset:';
  ...
  const progressPath = ['math', 'task12'];
  ...
  const renderNav = () => {
    prototypeNav.innerHTML = prototypes.map((proto) => {
      const saved = validSaved(getSaved(proto.id), proto);
      const correct = saved?.correctIds?.length || 0;
      return `<button type="button" class="prototype-progress ${prototypeState(proto)} ${proto.id === activeId ? 'is-current' : ''}" data-task12-prototype="${proto.id}"><b>${proto.id}</b><span>${correct}/${proto.items.length}</span></button>`;
    }).join('');
    prototypeNav.querySelectorAll('[data-task12-prototype]').forEach((button) => button.addEventListener('click', () => {
      activeId = button.dataset.task12Prototype;
      render();
  ...
  const checkPrototype = async () => {
    ...
    renderOverall();
    saveCloud(payload);
    if (typeof window.ym === 'function' && window.METRIKA_COUNTER_ID) window.ym(window.METRIKA_COUNTER_ID, 'reachGoal', 'MATH_TASK12_TEST');
  };
  const resetPrototype = async () => {
    ...
    if (cloudUser && resetKey && window.ogeSupabase) {
      ...
      } catch (error) { console.warn('Не удалось синхронизировать сброс прогресса №12', error); }
    }
