...
OUTPUT = PART_ONE / "task1-5-stoves-data.js"
...
def localize_drawings(markup):
    ...
    for source in sorted(set(re.findall(
        r'/drawings/FIPI_OGE_MATH/real_math/(stoves-\d+\.svg)', markup,
    ))):
...
def collect():
    ...
    heading = soup.find(id="subtopic-1ndash5_pechki")
    ...
    if len(analogs) != 2:
        raise RuntimeError(f"Expected 2 stove analogs, collected {len(analogs)}")
    return [{
        "id": "stoves-5.1",
        "number": "5.1",
        "title": "Печки",
...
if __name__ == "__main__":
    rows = collect()
    write_data(rows)
    print(f"Collected {len(rows[0]['analogs'])} stove analogs.")
